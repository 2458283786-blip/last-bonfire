...
func _run() -> void:
	...
	var house1: HousingBuilding = (load("res://scenes/buildings/house.tscn") as PackedScene).instantiate()
	add_child(house1)
	var house2: HousingBuilding = (load("res://scenes/buildings/house.tscn") as PackedScene).instantiate()
	add_child(house2)
	house1.assign_villager(v1)
	house1.assign_villager(v2)
	house2.assign_villager(v3)
	panel.open_panel()
