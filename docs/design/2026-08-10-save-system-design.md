...
> 版本：v0.2（2026-08-10，已实现）
> 状态：已按实施计划完成全部内容并通过全量测试；随开发迭代更新。
...
> 实现修正（v0.2）：实际恢复顺序为 清场 → 昼夜（静默）→ 建筑 → 资源 → 居民 → 玩家 → 经济（最后覆盖，避免仓库 `_ready` 扩容干扰存档容量）→ 城镇登记清空 → 恢复野生树生成器。加载期间 DayManager 静默恢复（不发射信号），`WildTreeSpawner.refill_enabled` 在加载期间置 false 抑制即时补树。
...
- 2026-08-10 v0.1：方案确认（自动+手动存档、单槽、清空重建）。
- 2026-08-10 v0.2：按计划实现并合入 `feature/save-system`——
  - SaveManager：JSON 单文件、版本 1、错误信号、路径可注入。
  - 昼夜/经济/玩家/建筑/资源/居民 六节快照与静默恢复；建筑补 `building_id`；伐木工加载后重新分配；野生树加载期抑制补足。
  - 时机：城镇内天亮自动存档；暂停菜单"存档/读档"；boot"继续游戏/新游戏"。
  - 测试：新增 6 个测试脚本（核心/全局/建筑/资源/居民/流程），全量 49 个测试通过。
