...
> 状态：已按确认版实现（2026-08-08），见 9.1 实现记录。

## 9.1 实现记录（2026-08-08）

- `EnemyData` + `enemy_night_wolf.tres`：夜狼（HP3、伤害1、优先级 villager/building/player、掉落金币/材料）。
- `Enemy`（CharacterBody2D，碰撞层 4）：IDLE → CHASE → ATTACK → DIE；攻击优先级选目标；死亡按掉落表生成 Pickup。
- 玩家：血条制（max_hp 100 + 受击无敌 1s），死亡回篝火复活；近战/箭矢命中敌人；走过拾取物自动拾取入库。
- 居民：受伤（非死亡）失去 N 日工作能力，释放职业名额，伤愈自动恢复。
- 建筑：统一 Building 基类（血量/损坏/重建），新建篝火核心建筑。
- 昼夜：DayManager 增加白天/黄昏/夜晚阶段（F10 调试推进，F9 推进一天），每 60 秒自动轮转。
- `NightWaveSpawner`：夜晚在城镇边缘生成一波怪物，强度随天数递增。
